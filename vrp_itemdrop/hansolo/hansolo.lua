...
AddEventHandler('DropSystem:remove',function(id)
	if dropList[id] ~= nil then
		if dropList[id].obj ~= nil then
			DeleteObject(dropList[id].obj)
		end
		dropList[id] = nil
	end
end)
	...
	while true do
		...
		for k,v in pairs(dropList) do
			...
			if distance <= 15 then
				...
				if v.obj == nil and v.objHash ~= nil and not cooldown then
					v.obj = CreateObject(GetHashKey(v.objHash), v.x, v.y, cdz+0.01, true, true, true)
				else
