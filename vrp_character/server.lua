...
function processSpawnController(source,statusSent,user_id)
	...
	elseif statusSent == 1 or statusSent == 0 then
		userlogin[user_id] = true
		TriggerClientEvent("vrp_character:characterCreate",source)
	end
end

RegisterServerEvent("vrp_character:finishedCharacter")
AddEventHandler("vrp_character:finishedCharacter",function(characterNome,characterSobrenome,characterAge,currentCharacterMode)
	local source = source
...
function doSpawnPlayer(source,user_id,firstspawn)
	TriggerClientEvent("vrp_character:normalSpawn",source,firstspawn)
