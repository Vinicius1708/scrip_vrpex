...
Citizen.CreateThread(function()
	for k,v in pairs(vRP.vehicleGlobal()) do
		...
		if v.tipo == "brasil" then
			local vehicle = vRP.query("creative/get_estoque",{ vehicle = k })
			if vehicle[1] ~= nil then
				local bau = inventory.chestweight[k] or 50
				table.insert(brasil,{ k = k, nome = v.name, price = v.price, chest = parseInt(bau), stock = parseInt(vehicle[1].quantidade) })
			end
		end
		if v.tipo == "caminhoes" then
			local vehicle = vRP.query("creative/get_estoque",{ vehicle = k })
			if vehicle[1] ~= nil then
				local bau = inventory.chestweight[k] or 50
				table.insert(caminhoes,{ k = k, nome = v.name, price = v.price, chest = parseInt(bau), stock = parseInt(vehicle[1].quantidade) })
			end
		end
...
function src.updateVehicles(vname,vehtype)
	...
	elseif vehtype == "brasil" then
		for k,v in pairs(brasil) do
			if v.k == vname then
				table.remove(brasil,k)
				local vehicle = vRP.query("creative/get_estoque",{ vehicle = vname })
				if vehicle[1] ~= nil then
					local bau = inventory.chestweight[vname] or 50
					table.insert(brasil,{ k = vname, nome = vRP.vehicleName(vname), price = (vRP.vehiclePrice(vname)), chest = parseInt(bau), stock = parseInt(vehicle[1].quantidade) })
				end
			end
		end
	elseif vehtype == "caminhoes" then
		for k,v in pairs(caminhoes) do
			if v.k == vname then
				table.remove(caminhoes,k)
				local vehicle = vRP.query("creative/get_estoque",{ vehicle = vname })
				if vehicle[1] ~= nil then
					local bau = inventory.chestweight[vname] or 50
					table.insert(caminhoes,{ k = vname, nome = vRP.vehicleName(vname), price = (vRP.vehiclePrice(vname)), chest = parseInt(bau), stock = parseInt(vehicle[1].quantidade) })
				end
			end
		end
	...
	if user_id then
		...
		if vehicle[1] then
			...
			if vRP.tryFullPayment(user_id,vRP.vehiclePrice(name)*0.85) then
				...
				TriggerClientEvent("Notify",source,"sucesso","Você comprou um <b>"..vRP.vehicleName(name).."</b> por <b>R$ "..vRP.format(parseInt(vRP.vehiclePrice(name)*0.85)).." reais</b>.",10000)
				...
				elseif vRP.vehicleType(name) == "brasil" then
					TriggerClientEvent('dealership:Update',source,'updatebrasil')
				elseif vRP.vehicleType(name) == "caminhoes" then
					TriggerClientEvent('dealership:Update',source,'updatecaminhoes')
